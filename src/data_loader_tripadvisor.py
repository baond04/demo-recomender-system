"""
data_loader_tripadvisor.py
Module nạp và tiền xử lý bộ dữ liệu TripAdvisor Hotel Reviews cho các mô hình RecSys (CB, MF, LightGCN, SimGCL).
"""
...
class DataLoaderTripAdvisor:
    def __init__(self, dataset_path, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1, min_rating=3.0, seed=42):
    ...
    def load_raw_data(self):
        ...
        if not parquet_file or not os.path.exists(parquet_file):
            raise FileNotFoundError(f"Không tìm thấy file Parquet dataset TripAdvisor tại {self.dataset_path}")
        ...
        preds = data_dict.get("prediction", [])
        ids = data_dict.get("id", [])
        n_rows = len(texts)

        # Trích xuất rating từ trường prediction
        ratings = []
        for p in preds:
            r = 4.0
        ...
        # Phân nhóm các bài đánh giá thành các Item đại diện (dựa trên cụm từ từ khóa khách sạn)
        # và sinh User ID tương tác để xây dựng ma trận tương tác User-Item
        interactions = []
        ...
        for idx in range(n_rows):
            ...
            raw_id = str(ids[idx]) if idx < len(ids) else f"review_{idx}"

            # Phân bổ item_id dựa trên cụm từ địa danh/khách sạn trong bài đánh giá
            # để nhóm các bài review cùng loại khách sạn thành 1 Item
            first_words = txt.strip().lower().split()[:3]
            item_key = "_".join(first_words) if len(first_words) >= 2 else f"hotel_{idx % 150}"

            # Tạo mã giả lập User theo chu kỳ mẫu thử nghiệm (giúp tạo tương tác đồ thị dày đặc)
            raw_user_id = f"user_{idx % 500}"

            if item_key not in self.item2id:
                i_idx = len(self.item2id)
                self.item2id[item_key] = i_idx
                self.id2item[i_idx] = item_key

            if raw_user_id not in self.user2id:
                u_idx = len(self.user2id)
                self.user2id[raw_user_id] = u_idx
                self.id2user[u_idx] = raw_user_id

            u_id = self.user2id[raw_user_id]
            i_id = self.item2id[item_key]
    ...
    def build_tfidf_features(self, num_items):
        ...
        if not top_words:
            top_words = ["hotel", "room", "staff", "service", "clean", "location", "stay", "great"]
